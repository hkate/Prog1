...
int main()
{
    ...
    Lines grid;
    int x_size = 800; //x tengely max szélessége? kezdő érték?
    int y_size = 800;   //y tengely max szélessége?
    for (int i=100; i<=x_size; i+=100) {
                    grid.add(Point{i, 0}, Point{i, y_size});       //horizontal line
                    grid.add (Point{0, i}, Point{x_size, i});      //vertical line
    }
    win.attach(grid);
    win.wait_for_button() ;
    ...
    Vector_ref<Rectangle> rectangles;           //Vector_ref -> kitörli azokat a válozókat a közös memóriaterületről amik beragadnánk a new miatt (egyébként kézzel kéne őket törölni)
    ...
    while (true)    //random x-et és y-t kérek be
        {
            int randx = rand() % 7 + 1;     // v2 in the range 1 to 8
            randx *= 100;       //beszorzom százzal, hogy illeszkedjen a rácshoz
